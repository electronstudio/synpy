...
def recursive_tree(binarySequence, subdivisionSequence, weightSequence, metricalWeight, level, Lmax):
    ...
    # If matching to a Rest type, add to terminal nodes
        ...
        print(
            "WARNING: LHL tree recursion descended to Lmax, returning a note node but result will not be fully accurate.  Check the rhythm pattern under test and/or specify larger Lmax to rectify the problem.")
        output.append(Node('N', metricalWeight))
    ...
    # Keep subdividing by the subdivisor of the next level
        ...
        subWeightSequences = concatenate([metricalWeight],
                                         repeat([weightSequence[level + 1]], subdivisionSequence[level + 1] - 1))
        for a in range(len(subBinarySequences)):
            output = output + recursive_tree(subBinarySequences[a], subdivisionSequence, weightSequence,
                                             subWeightSequences[a], level + 1, Lmax)
...
def get_syncopation(bar, parameters=None):
    ...
    if get_rhythm_category(binarySequence, subdivisionSequence) == 'poly':
        ...
        if not are_parameters_valid(Lmax, weightSequence, subdivisionSequence):
            ...
            terminalNodes = recursive_tree(ceiling(binarySequence), subdivisionSequence, weightSequence,
                                           weightSequence[0], 0, Lmax)
            ...
            for i in range(len(terminalNodes) - 1, 0, -1):
                if terminalNodes[i].nodeType == 'R':
                    for j in range(i - 1, -1, -1):
                        if (terminalNodes[j].nodeType == 'N') & (
                                terminalNodes[i].metricalWeight >= terminalNodes[j].metricalWeight):
