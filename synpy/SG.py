...
def get_syncopation(bar, parameters=None):
    ...
    if get_rhythm_category(velocitySequence, subdivisionSequence) == 'poly':
        ...
        velocitySequence = velocity_sequence_to_min_timespan(
            velocitySequence)  # converting to the minimum time-span format
            ...
            if Lmax != None:
                ...
                def ave_dif_neighbours(index, level):
                    ...
                    def find_pre(index, level):
                        preIndex = (index - 1) % len(
                            H)  # using % is to restrict the index varies within range(0, len(H))
                    ...
                    for l in range(level, max(H) + 1):
                        ave = (parameterGarma * dif(index, find_pre(index, l)) + dif(index, find_post(index, l))) / (
                                    1 + parameterGarma)
                        averages.append(ave)
                    return averages
                ...
                if velocitySequence != None:
                    ...
                    print(
                        'Try giving a bigger Lmax so that the rhythm sequence can be measured by the matching metrical weights sequence (H).')
    return syncopation
