...
def is_time_signature_valid(timeSignature, subdivisionSequence, beatLevel):
    isValid = False
    if ('/' not in timeSignature) or (not timeSignature.split('/')[0].isdigit()) or (
    not timeSignature.split('/')[1].isdigit()):
        ...
        if isinstance(Lmax, int) and Lmax > 0:
            if Lmax <= len(subdivisionSequence) - 1:
                if Lmax <= len(weightSequence) - 1:
                    ...
                    print(
                        'Error: Lmax exceeds the length of weight-sequence. Either reduce Lmax, or provide a new weight-sequence whose length is greater or equal to Lmax.')
            else:
                print(
                    'Error: Lmax exceeds the length of subdivision-sequence. Either reduce Lmax, or extend subdivision-sequence through updating time-signature (refer to update_time_signature function).')
        else:
